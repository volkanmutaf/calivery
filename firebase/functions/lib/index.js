...
exports.migrateTenantData = exports.deleteTenant = exports.updateTenantStatus = exports.createTenantAdmin = exports.updateTenantConfig = exports.createTenant = exports.setTenantClaims = exports.adminCompleteOrder = exports.deleteOrder = exports.getUserByUsername = exports.deleteUser = exports.updateUser = exports.createUser = exports.triggerEarningsRecalculation = exports.recalculateEarnings = exports.driverCompleteStep = exports.publishRouteGroup = exports.manualAssignOrders = exports.autoAssignOrders = exports.createOrder = void 0;
...
function getTenantId(auth) {
    if (!auth)
        return 'default';
    return auth.token?.tenant_id || 'default';
}
/**
 * Verify user is a driver
 */
async function verifyDriver(uid) {
    if (!uid) {
        throw new https_1.HttpsError('unauthenticated', 'Must be logged in');
    }
    const role = await getUserRole(uid);
    if (role !== 'driver') {
        throw new https_1.HttpsError('permission-denied', 'Driver access required');
    }
}
/**
 ...
 */
async function writeOrderEvent(orderId, eventType, actorRole, actorId, metadata = {}) {
 ...
 */
exports.createOrder = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    ...
    // Write audit event
    await writeOrderEvent(orderRef.id, 'order_created', 'admin', request.auth.uid, { order_code: orderCode });
    return {
 ...
 */
exports.autoAssignOrders = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    try {
        await verifyAdmin(request.auth?.uid);
        const data = request.data;
        ...
        if (driversSnap.empty) {
            ...
            throw new https_1.HttpsError('failed-precondition', 'No active drivers available');
        }
        ...
        if (drivers.length === 0) {
            throw new https_1.HttpsError('failed-precondition', 'No selected drivers available');
        }
        ...
        orders.sort((a, b) => {
            const timeA = a.time_window_start ? a.time_window_start.toDate?.().getTime() || 0 : 0;
            const timeB = b.time_window_start ? b.time_window_start.toDate?.().getTime() || 0 : 0;
            return timeA - timeB;
        });
        ...
        for (const [driverId, assignedOrders] of driverAssignments) {
            ...
            let steps;
            try {
                steps = (0, routing_1.generateRouteSteps)(driver.driver_base_lat, driver.driver_base_lng, assignedOrders);
            }
            catch (routeError) {
            ...
            let totalDistance = 0;
            if (steps.length > 0) {
                totalDistance = (0, routing_1.haversineDistance)(driver.driver_base_lat, driver.driver_base_lng, steps[0].lat, steps[0].lng);
                for (let i = 1; i < steps.length; i++) {
                    totalDistance += (0, routing_1.haversineDistance)(steps[i - 1].lat, steps[i - 1].lng, steps[i].lat, steps[i].lng);
                }
            }
            ...
            for (const step of steps) {
                const distToStep = (0, routing_1.haversineDistance)(prevLat, prevLng, step.lat, step.lng);
            ...
            if (previewOnly) {
                ...
                const routeGroup = {
                    ...
                    scheduled_date: data.scheduled_date,
                    status: 'active',
                    algorithm_version: routing_1.ALGORITHM_VERSION,
                    generated_at: now,
                    published_at: null,
                    summary: {
                ...
                // Write audit events
                for (const order of assignedOrders) {
                    await writeOrderEvent(order.id, 'assigned', 'system', 'system', {
                        driver_id: driverId,
                        driver_username: driver.username,
                        route_group_id: routeGroupRef.id,
                        algorithm_version: routing_1.ALGORITHM_VERSION,
        ...
        throw new https_1.HttpsError('internal', `Assignment failed: ${error.message}`);
    }
});
/**
 ...
 * Supports preview_only mode for showing preview before confirming.
 */
exports.manualAssignOrders = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    try {
        await verifyAdmin(request.auth?.uid);
        ...
        if (!scheduled_date || !order_ids || order_ids.length === 0 || !driver_id) {
            throw new https_1.HttpsError('invalid-argument', 'Missing required fields: scheduled_date, order_ids, driver_id');
        }
        ...
        if (driver.role !== 'driver') {
            throw new https_1.HttpsError('invalid-argument', 'Selected user is not a driver');
        }
        if (tenantId && tenantId !== 'default' && driver.tenant_id !== tenantId) {
            throw new https_1.HttpsError('permission-denied', `Driver does not belong to tenant ${tenantId}`);
        }
        ...
        for (const orderId of order_ids) {
            ...
            if (orderData.status !== 'new') {
                throw new https_1.HttpsError('failed-precondition', `Order ${orderId} is already assigned or completed`);
            }
            if (tenantId && tenantId !== 'default' && orderData.tenant_id !== tenantId) {
                throw new https_1.HttpsError('permission-denied', `Order ${orderId} does not belong to tenant ${tenantId}`);
            }
        ...
        const steps = (0, routing_1.generateRouteSteps)(driver.driver_base_lat, driver.driver_base_lng, orders);
        // Calculate total distance
        let totalDistance = 0;
        if (steps.length > 0) {
            totalDistance = (0, routing_1.haversineDistance)(driver.driver_base_lat, driver.driver_base_lng, steps[0].lat, steps[0].lng);
        ...
        const routeGroup = {
            ...
            status: 'active',
            algorithm_version: routing_1.ALGORITHM_VERSION + '-manual',
            generated_at: now,
            published_at: null,
            summary: {
                distance_estimate: Math.round(totalDistance * 10) / 10,
        ...
        // Write audit events
        for (const order of orders) {
            await writeOrderEvent(order.id, 'assigned', 'admin', request.auth.uid, {
                driver_id: driver_id,
                driver_username: driver.username,
                route_group_id: routeGroupRef.id,
                algorithm_version: routing_1.ALGORITHM_VERSION + '-manual',
        ...
        throw new https_1.HttpsError('internal', `Manual assignment failed: ${error.message}`);
    }
});
/**
 * Publish a route group to the driver (Admin only)
 */
exports.publishRouteGroup = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
    ...
    if (!routeGroupDoc.exists) {
        throw new https_1.HttpsError('not-found', 'Route group not found');
    }
    const now = admin.firestore.Timestamp.now();
    // Update route group
    ...
    for (const orderId of orderIds) {
        await writeOrderEvent(orderId, 'system_route_generated', 'system', 'system', {
            route_group_id,
 ...
 */
exports.driverCompleteStep = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    try {
        ...
        const routeGroup = routeGroupDoc.data();
        if (routeGroup.driver_id !== driverId) {
            throw new https_1.HttpsError('permission-denied', 'This route is not assigned to you');
        }
        // Get the step
        const stepRef = routeGroupRef.collection('steps').doc(step_id);
        const stepDoc = await stepRef.get();
        if (!stepDoc.exists) {
            throw new https_1.HttpsError('not-found', 'Step not found');
        }
        ...
        if (pendingStepsSnap.empty || pendingStepsSnap.docs[0].id !== step_id) {
            throw new https_1.HttpsError('failed-precondition', 'This is not the current step');
        }
        ...
        throw new https_1.HttpsError('internal', `Failed to complete step: ${error.message}`);
    }
});
/**
 * Recalculate weekly earnings for all drivers (scheduled daily)
 */
exports.recalculateEarnings = (0, scheduler_1.onSchedule)({ schedule: '0 2 * * *', region: 'us-central1' }, // 2 AM daily
async () => {
    const weekStart = (0, routing_1.getWeekStartDate)();
 ...
 */
exports.triggerEarningsRecalculation = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
 ...
 */
exports.createUser = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
    const data = request.data;
    const callerTenantId = getTenantId(request.auth);
    ...
    if (callerTenantId === 'default' && data.tenant_id) {
        ...
        throw new https_1.HttpsError('permission-denied', 'Cannot create user outside of your tenant');
    }
    // Validate required fields
    if (!data.email || !data.password || !data.username || !data.role) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required fields: email, password, username, role');
    }
 ...
 */
exports.updateUser = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
    ...
    if (callerTenantId !== 'default' && profileData.tenant_id !== callerTenantId) {
        throw new https_1.HttpsError('permission-denied', 'Cannot update user outside of your tenant');
    }
    try {
        ...
        if (updates.email || updates.password) {
            ...
            if (updates.username)
                authUpdate.displayName = updates.username;
        ...
        if (updates.username !== undefined)
            profileUpdate.username = updates.username;
        if (updates.email !== undefined)
            profileUpdate.email = updates.email;
        if (updates.phone !== undefined)
            profileUpdate.phone = updates.phone;
        if (updates.role !== undefined)
            profileUpdate.role = updates.role;
        if (updates.is_active !== undefined)
            profileUpdate.is_active = updates.is_active;
        if (updates.driver_base_address !== undefined)
            profileUpdate.driver_base_address = updates.driver_base_address;
        if (updates.driver_base_lat !== undefined)
            profileUpdate.driver_base_lat = updates.driver_base_lat;
        if (updates.driver_base_lng !== undefined)
            profileUpdate.driver_base_lng = updates.driver_base_lng;
        ...
        const err = error;
        throw new https_1.HttpsError('internal', err.message || 'Failed to update user');
    }
});
/**
 ...
 */
exports.deleteUser = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
    ...
    if (profileDoc.exists) {
        const profileData = profileDoc.data();
        if (callerTenantId !== 'default' && profileData.tenant_id !== callerTenantId) {
            throw new https_1.HttpsError('permission-denied', 'Cannot delete user outside of your tenant');
        }
    }
 ...
 */
exports.getUserByUsername = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
 ...
 */
exports.deleteOrder = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
    ...
    if (callerTenantId !== 'default' && orderData.tenant_id !== callerTenantId) {
        throw new https_1.HttpsError('permission-denied', 'Cannot access order outside of your tenant');
    }
 ...
 */
exports.adminCompleteOrder = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifyAdmin(request.auth?.uid);
    const { order_id } = request.data;
    const callerTenantId = getTenantId(request.auth);
    if (!order_id) {
    ...
    // Write audit event
    await writeOrderEvent(order_id, 'delivered', 'admin', request.auth.uid, { method: 'force_complete' });
    return { success: true };
});
// ============================================
 ...
 * Super admin only
 */
exports.setTenantClaims = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
    const { uid, tenant_id, role } = request.data;
    if (!uid || !tenant_id || !role) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required fields: uid, tenant_id, role');
    }
    // Set custom claims
    await admin.auth().setCustomUserClaims(uid, {
        tenant_id,
 ...
 * Super admin only
 */
exports.createTenant = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
    const data = request.data;
    if (!data.tenant_id || !data.name || !data.brand_name) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required fields: tenant_id, name, brand_name');
    }
    // Check if tenant already exists
    const existingTenant = await db.collection('tenants').doc(data.tenant_id).get();
    if (existingTenant.exists) {
        throw new https_1.HttpsError('already-exists', 'Tenant with this ID already exists');
    }
    const now = admin.firestore.Timestamp.now();
 ...
 * Super admin only
 */
exports.updateTenantConfig = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
    const { tenant_id, features, limits } = request.data;
    if (!tenant_id) {
        throw new https_1.HttpsError('invalid-argument', 'tenant_id is required');
    }
 ...
 * Super admin only
 */
exports.createTenantAdmin = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
    const { email, password, username, tenant_id, role } = request.data;
    // Validate inputs
    if (!email || !password || !username || !tenant_id || !role) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required fields: email, password, username, tenant_id, role');
    }
    if (!['tenant_admin', 'dispatcher', 'driver'].includes(role)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid role. Must be tenant_admin, dispatcher, or driver');
    }
    ...
    const tenantDoc = await db.collection('tenants').doc(tenant_id).get();
    if (!tenantDoc.exists) {
        throw new https_1.HttpsError('not-found', `Tenant '${tenant_id}' not found`);
    }
    ...
    if (!usernameSnap.empty) {
        throw new https_1.HttpsError('already-exists', 'A user with this username already exists');
    }
 ...
 * Super admin only
 */
exports.updateTenantStatus = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
    const { tenant_id, status } = request.data;
    if (!tenant_id || !status) {
        throw new https_1.HttpsError('invalid-argument', 'tenant_id and status are required');
    }
    if (!['active', 'suspended', 'inactive'].includes(status)) {
        throw new https_1.HttpsError('invalid-argument', 'Invalid status. Must be active, suspended, or inactive');
    }
 ...
 * Super admin only
 */
exports.deleteTenant = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
    const { tenant_id } = request.data;
    if (!tenant_id) {
        throw new https_1.HttpsError('invalid-argument', 'tenant_id is required');
    }
    const tenantRef = db.collection('tenants').doc(tenant_id);
 ...
 * Super admin only
 */
exports.migrateTenantData = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    await verifySuperAdmin(request.auth?.uid);
