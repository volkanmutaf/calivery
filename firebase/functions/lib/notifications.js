...
exports.pickupReminderCron = exports.onOrderAssignedNotification = exports.sendManualNotification = void 0;
...
async function verifyAdminOrDispatcher(uid) {
    if (!uid) {
    ...
    if (role !== 'admin' && role !== 'super_admin' && role !== 'tenant_admin' && role !== 'dispatcher') {
        throw new https_1.HttpsError('permission-denied', 'Admin or Dispatcher access required');
    }
}
/**
 * Common function to send a multicast notification via FCM and Expo
 */
async function sendPushNotification(tenantId, tokens, title, body, data = {}) {
    if (tokens.length === 0)
        return { successCount: 0, failureCount: 0 };
    const expo = await getExpo();
    const { Expo } = await Promise.resolve().then(() => __importStar(require('expo-server-sdk'))); // Import class reference safely
 ...
 */
exports.sendManualNotification = (0, https_1.onCall)({ region: 'us-central1', cors: true }, async (request) => {
    ...
    if (payload.tenant_id !== tenantId && tenantId !== 'default' && !(request.auth?.token.super_admin)) {
        throw new https_1.HttpsError('permission-denied', 'Cross-tenant notifications not allowed');
    }
    ...
    if (payload.target_type === 'specific_driver') {
        if (!payload.target_driver_id) {
            throw new https_1.HttpsError('invalid-argument', 'target_driver_id is required for specific_driver');
        }
    ...
    const result = await sendPushNotification(effectiveTenantId, uniqueTokens, payload.title, payload.body, { type: 'manual' });
    // Record history
 ...
 */
exports.onOrderAssignedNotification = (0, firestore_1.onDocumentUpdated)({ document: 'orders/{orderId}', region: 'us-central1' }, async (event) => {
 ...
 */
exports.pickupReminderCron = (0, scheduler_1.onSchedule)({ schedule: 'every 5 minutes', region: 'us-central1' }, async (event) => {
    const now = admin.firestore.Timestamp.now();
            ...
            if (timeDiffMins > 0 && timeDiffMins <= 60) {
                ...
                devicesSnap.docs.forEach(d => {
                    ...
                    if (data.expo_push_token)
                        tokens.push(data.expo_push_token);
